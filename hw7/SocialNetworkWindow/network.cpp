...
int Network::readUsers(char* fname){
    ...
    for (int i = 0; i < num_users; i++) {
        ...
        // 3) Read the name line and trim leading whitespace
        std::string line;
        ...
        // 4) Read the year line and parse
        std::getline(infile, line);
        {
            auto idx = line.find_first_not_of(" \t");
            std::istringstream(line.substr(idx == std::string::npos ? 0 : idx))
                >> year;
        }

        // 5) Read the zip line and parse
        std::getline(infile, line);
        {
            auto idx = line.find_first_not_of(" \t");
            std::istringstream(line.substr(idx == std::string::npos ? 0 : idx))
        ...
        // 6) Read the friends list
        std::getline(infile, line);
        {
            auto idx = line.find_first_not_of(" \t");
            std::istringstream ss(line.substr(idx == std::string::npos ? 0 : idx));
            int fid;
            while (ss >> fid) friends.insert(fid);
        }

        // 7) Now year and zip are in scope and initialized!
...
int Network::readPosts(char* fname){
    ...
    while (std::getline(infile, line)){
        if(line.find("REACTION")==0){
            ...
            char colon;
            ss>>messageId>>colon >>userId>>colon>>reactionType;
            ...
            if (targetPost){
                targetPost->addReaction(userId, reactionType);
            }
        }
    }
...
int Network::writePosts(char* fname){
    ...
    for (Post* post : allPosts){
        ...
        for (const ReactionType& type : types){
            const std::set<int>& users= post->getUsersWithReaction(type.getId());
            for (int userId : users){
                outfile<< "REACTION:" <<post->getMessageId()<<":"<<userId<<":" <<type.getId()<<std::endl;
            }
        }
    }
...
std::vector<Post*> Network::getAllPosts(){
    std::vector<Post*> allPosts;
    for (User* user : users_){
        std::vector<Post*> userPosts = user->getPosts();
        allPosts.insert(allPosts.end(), userPosts.begin(), userPosts.end());
...
std::vector<Post*> Network::getVisiblePosts(int userId){
    std::vector<Post*> visiblePosts;
    std::vector<Post*> allPosts = getAllPosts();

    for (Post* post: allPosts){
        if (post->getIsPublic()|| post->getOwnerId() == userId){
...
std::vector<User*> Network::getUsersByReaction(int postId,std::string reactionType){
    std::vector<User*> users;

    Post* targetPost = nullptr;
    for(User* user:users_){
        for(Post* post : user->getPosts()){
            if (post->getMessageId() ==postId){
                targetPost = post;
                break;
            }
        }
    ...
    if (targetPost){
        const std::set<int>& userIds = targetPost->getUsersWithReaction(reactionType);
        for(int userId : userIds){
            if(userId<users_.size()){
                users.push_back(users_[userId]);
            }
        }
    }
    return users;
